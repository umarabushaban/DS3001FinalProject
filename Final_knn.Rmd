...
```
Our KNN model confusion matrix produces a 90 % accuracy rate of predicting a highly rated move (a movie with an average vote above 7.5), which is a great improvement from the baserate of 8.1%. The sensitivity or true positive rate, when highly rated movies are accurately classified, is 4.1%, which is very low. However, the model's specificity is 98%, and false positive rate is 100% - 98% = 2%, which is very low, meaning the model rarely inaccurately classifies poorly rated movies as highly rated ones. 
